...
static void unpack_gc(struct image *out, u8_t *in)
{
...
static u8_t *expand_lzss(u8_t *lzss)
{
    ...
    struct image *images, int nimages)
{
    ...
    snprintf(filename, sizeof(filename), "%s/system.idx", in_dir);
...
int main(int argc, char **argv)
{
    ...
    for (i = 0 ; i < 32 ; i++) {
        ...
        snprintf(filename, sizeof(filename), "%s/%d.gcz", in_dir, i);
        f = fopen(filename, "rb");
        if (f == NULL) break;
