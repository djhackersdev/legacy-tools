...
void loadData(FILE* infile, unsigned char data[33554432], char* filename);
void printUsage();
...
int main(int argc, char* argv[])
{
    ...
    unsigned char *data;
    data=(unsigned char *)malloc(33554432*sizeof(unsigned char));
...
//        data[b] = fgetc(infile);
    fread(data, 1, 33554432, infile);

    fclose(infile);

    for(c = 0; c < numberOfEdits; c++)
    {
...
//        fprintf(outfile, "%c", data[zz]);
    fwrite(data, 1, 33554432, outfile);
...
void loadData(FILE* infile, unsigned char data[33554432], char* filename)
{
    ...
    unsigned char inputData[8192];
    unsigned char outputData[4096];
